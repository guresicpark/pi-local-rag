...
- Lower indexing memory — line offsets live in growable `Int32Array`s (4 B/line vs 8-byte tagged SMIs in a `number[]`), and embedding vectors stay packed as `Float32Array` end-to-end instead of being boxed to `number[]` at embed time and converted back to float32 at insert time (~102 MB → ~44 MB retained for a 14k-chunk store; 2–6.6× less transient GC garbage per file)
